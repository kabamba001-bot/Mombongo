...
async function run(){
  ...
  for(const userDoc of usersSnap.docs){
    ...
    for(const store of stores){
      ...
      const { lowStock, expired, expiringSoon } = computeStoreAlerts(storeData);
      if(lowStock.length === 0 && expired.length === 0 && expiringSoon.length === 0) continue;

      const { title, body } = buildMessage(store.name, lowStock, expired, expiringSoon, '🔔 Rappel —');
